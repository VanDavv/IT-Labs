...
public:
    static Game* init() {
        auto game = new Game();
        game->drivers.push_back(new Driver());
        game->cars.push_back(new CheapCar());
        return game;
    }
    void buyDriver() {
    ...
    void buyCar() {
        ...
        switch(car_quality) {
            case 1:
                cars.push_back(new CheapCar());
                break;
            case 2:
                cars.push_back(new CommonCar());
                break;
            case 3:
                cars.push_back(new ExpensiveCar());
                break;
            default:
                break;
        }
    ...
    void race() {
        int min_count = std::min(Car::getCarCount(), Driver::getDriverCount());
        ...
        for (int i = 0; i < min_count; i++) {
            ...
            bool exists = false;
            mymap::iterator it;
            for (it = assigned_car.begin(); it != assigned_car.end(); ++it )
                if (it->second == choice)
                    exists = true;
